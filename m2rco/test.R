...
ca_mob <- ca_mob %>%
  ...
  complete(sub_region_1, date) %>%
  distinct() %>%
  fill(colnames(ca_mob[,2:12]), .direction = "updown")

colnames(ca_mob)[1] <- "state_name"

dd1 <- merge(dd, ca_mob, by=c("date", "state_name"))

y <- dim(dd1)[1]
for (i in 1:y) {
  if (dd1$date[i]!=dd2$date[i]) {
    dd1 <- dd1[-i,]
  }
}
...
for (i in 1:58) {
  s <- subset(dd1, state_name==states[i])
  X_partial[[i]] <- as.matrix(s[, c(20,16,18)], ncol=3)
}
