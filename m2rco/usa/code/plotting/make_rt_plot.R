...
make_rt_point_plot <- function(rt_data_long, label, JOBID, ext = ".png"){
  ...
  rt_data_end$grouping <- factor(rt_data_end$grouping)
  p1 <- ggplot(rt_data_end) + 
    geom_point(aes(x = state_name, y = rt, col = grouping),  stat="identity") + 
  ...
  rt_data_start$grouping <- factor(rt_data_start$grouping)
  ...
  p3 <- ggplot(rt_data_long) + 
    geom_point(aes(y = state_name, x = rt, shape = x, col = grouping),  stat="identity", 
               position=position_dodge(width = -0.5)) + 
    geom_pointrangeh(aes(x = rt, xmin=rt_min, xmax=rt_max, y=state_name, group = x, col = grouping),
  ...
  #  scale_x_log10(expand = c(0, 0),breaks=0:6) + 
    ...
    theme(axis.line = element_line(colour = "black"), 
          ...
          panel.background = element_blank(),
          legend.key = element_blank())
  ggsave(paste0("usa/figures/rt_point_", "_", ext), p3, width = 8, height=7)
}
